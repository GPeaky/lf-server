...
const Transfer = async (to, amount, playerId) => {
    ...
    mp.crypto.contract.methods.sendRewardsToPlayer(to, amount).send({from : mp.crypto.account.address, gas: 300000, nonce: cachedNonce})
        ...
        .on("error", async (error) => {
            console.log(`Error for ID: ${playerId}-${cachedNonce}`, error);
            User.status = 'failed'
            User.save()
        });
}
...
const Init = async () => {
    ...
    mp.crypto.nonce = await mp.crypto.web3.eth.getTransactionCount(mp.crypto.account.address) - 1
    // for (let i = 0; i < 2; i++) {
    //     setTimeout(() => {
    //         Transfer('0x9D64847BFBEAA3F59b798Ef0927045a611cfC3a1', mp.crypto.web3.utils.toWei('1', 'ether'), 'NACHO')
    //     },  2 * i);
    // }
    Transfer('0x9D64847BFBEAA3F59b798Ef0927045a611cfC3a1', mp.crypto.web3.utils.toWei('1', 'ether'), 'NACHO')
    Transfer('0x9D64847BFBEAA3F59b798Ef0927045a611cfC3a1', mp.crypto.web3.utils.toWei('1', 'ether'), 'NACHO')
    Transfer('0x9D64847BFBEAA3F59b798Ef0927045a611cfC3a1', mp.crypto.web3.utils.toWei('1', 'ether'), 'NACHO')
    Transfer('0xeFB0eB0911726cd6ed3B4b3e3d35793d4fC518fE', mp.crypto.web3.utils.toWei('1', 'ether'), 'PEAKY')
    Transfer('0xeFB0eB0911726cd6ed3B4b3e3d35793d4fC518fE', mp.crypto.web3.utils.toWei('1', 'ether'), 'PEAKY')
    Transfer('0xeFB0eB0911726cd6ed3B4b3e3d35793d4fC518fE', mp.crypto.web3.utils.toWei('1', 'ether'), 'PEAKY')
    Transfer('0xeFB0eB0911726cd6ed3B4b3e3d35793d4fC518fE', mp.crypto.web3.utils.toWei('1', 'ether'), 'PEAKY')
}
